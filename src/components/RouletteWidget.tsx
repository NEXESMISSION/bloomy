...
export default function RouletteWidget({ prizes, isLoggedIn }: { prizes: RoulettePrize[]; isLoggedIn: boolean }) {
  ...
  // Verrouille le scroll de la page tant que la popup est ouverte (évite la
  // barre de défilement qui apparaît/disparaît pendant l'animation).
  useEffect(() => {
    if (!open) return;
        ...
        {open && (
          ...
          >
            ...
            <motion.div
              ...
              className="relative z-10 flex max-h-[90vh] w-full max-w-md flex-col overflow-hidden rounded-3xl bg-paper shadow-pop"
            >
              <button onClick={close} className="absolute right-4 top-4 z-10 grid h-9 w-9 place-items-center rounded-full border border-line bg-paper/80 text-ink backdrop-blur hover:bg-sand" aria-label="Fermer">
                <X className="h-5 w-5" />
              </button>
              ...
              <div className="overflow-y-auto p-6 sm:p-8">
                ...
                <div className="relative mx-auto mt-6 h-60 w-60">
                  <div className="absolute left-1/2 top-[-4px] z-10 -translate-x-1/2 drop-shadow-sm">
                    <div className="h-0 w-0 border-l-[9px] border-r-[9px] border-t-[15px] border-l-transparent border-r-transparent border-t-ink" />
                  </div>
                  <div
                    className="h-full w-full rounded-full ring-4 ring-ink/90"
                    style={{
                  ...
                  <div className="absolute left-1/2 top-1/2 grid h-12 w-12 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full bg-ink text-white shadow-card ring-4 ring-paper">
                    <Gift className="h-5 w-5" />
                  </div>
                </div>

                {/* Légende : chaque couleur correspond à un lot (jamais de texte à l'envers). */}
