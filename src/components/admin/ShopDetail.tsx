...
import { Minus, Plus, Loader2, PackagePlus, RotateCcw, Trash2, Coins, Phone, MapPin, History, Boxes, Check } from "lucide-react";
...
function CountPanel({ shop, placement }: { shop: CShop; placement: Placement }) {
  ...
  const save = () =>
    start(async () => {
      ...
      if (res.ok) { setDone(true); router.refresh(); } else alert(res.error);
    });
  ...
  return (
    <div className="rounded-2xl border border-line bg-white p-5">
      ...
      <label className="mt-4 block max-w-xs">
        <span className="mb-1 block text-xs font-medium text-muted">Montant encaissé (auto si vide)</span>
      ...
      <div className="mt-5 flex flex-wrap gap-3">
        <button onClick={save} disabled={pending} className="btn-primary flex-1 sm:flex-none">
          {pending ? <Loader2 className="h-4 w-4 animate-spin" /> : done ? <><Check className="h-4 w-4" /> Enregistré</> : <><Coins className="h-4 w-4" /> Enregistrer la visite</>}
        </button>
  ...
  return (
    <div className="flex items-center rounded-full border border-line bg-white">
      ...
      <input
        ...
        inputMode="numeric"
        className="w-10 border-0 bg-transparent text-center text-base font-bold text-ink outline-none"
      />
